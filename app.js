...
app.use(session({'secret':'Bizim Ders'}))
...
app.get("/", function (req, res) {
    if (req.session.isim) return res.render('./index', {bilgiler: req.session.isim})
    res.render('./giris')
})
...
var pass="selam"
    ...
    kulanici = req.body.username;  // formdan gelen
    var sifre = req.body.password;     // formdan gelen
    
//username=req.body["username"]      // formdan gelen
//var password=req.body["password"]  // formdan gelen

if (user==kulanici && pass==sifre) 
 {
     req.session.isim=kulanici
   //  global.kisi=kulanici
     res.render('./index')
 } else {
     res.render('./giris')
 }
...
app.get('/kitaplar', function (req, res) {
    ...
    if (req.session.isim) return res.render('./kitaplar', {
        bilgiler: req.session.isim
    });
    res.render('./giris');
});    
app.get('/ogrenciler', function (req, res) {
    if (req.session.isim) return res.render('./ogrenciler', {
        bilgiler: req.session.isim
    });
    res.render('./giris');
});
app.get('/islemler', function (req, res) {
    if (req.session.isim) return res.render('./islemler', {
        bilgiler: req.session.isim
    });
    res.render('./giris');
});
app.get('/hakkimizda', function (req, res) {
    if (req.session.isim) return res.render('./hakkimizda', {
        bilgiler: req.session.isim
    });
    res.render('./giris');
});
app.get('/iletisim', function (req, res) {
    if (req.session.isim) return res.render('./iletisim', {
        bilgiler: req.session.isim
    });
    res.render('./giris');
});
...
app.get('/kuran', function (req, res) {
    if (req.session.isim) return res.render('./kuran', {
        bilgiler: req.session.isim
    });
    res.render('./giris');
});
...
app.get('/cikis', function (req, res) {
    delete req.session.isim
    //req.session.destroy();
    //req.cookieSession.destroy();
    res.render('./giris');
});
